...
class OptKmerSize{
	...
	static void multiThreading(int pool_size, int k_mer_len1, int k_mer_len2, BufferedWriter bw) { 
        ExecutorService executorServiceLater = Executors.newFixedThreadPool(pool_size); 
        int i, j;
		for (i = k_mer_len1; i < k_mer_len2; i++) {
			for (j = 1; j < seq_num; j++) {  
				executorServiceLater.execute(new ExtThread(i, j , bw));
			}
		}
	...
	static void codeMatch(int seq_num, GenSequence seq, int kmer_len , BufferedWriter bw)throws IOException { 
	...
	static void findOptKmerSize(String path, int k_mer_len1, int k_mer_len2, int pool_size)throws IOException{ 
		...
		BufferedWriter bw; 
		bw = new BufferedWriter(new FileWriter(kmer_file));
		multiThreading(pool_size, k_mer_len1, k_mer_len2, bw);
		bw.flush();
	} 
}