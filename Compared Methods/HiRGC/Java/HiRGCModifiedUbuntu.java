...
class HiRGCModifiedUbuntu {

    static final byte[] code_rule = {0, 1, 2, 3};
    static final char[] invert_code_rule = {'A', 'C', 'G', 'T'};
    static final int SUB_STR_LEN = 10; // 20, k-mer 
    static final int MAX_CHAR_NUM = 1<<20;			//1<<28,1<<30
    static final int MAX_ARR_NUM_BIT = 20; //30
    ...
    public HiRGCModifiedUbuntu() throws IOException {
        // result_text = "";
    }
    ...
    public static void readTarFile(String tarFile) throws IOException {
		...
		for (int i = 0; i < pos_vec_len; i++) {
			System.out.printf("(%d %d)\t", pos_vec_begin[i], pos_vec_length[i]);
        }
		System.out.println();
		
		for (int i = 0; i < n_vec_len; i++) {
			System.out.printf("(%d %d)\t", n_vec_begin[i], n_vec_length[i]);
        }
		System.out.println();
		
		for (int i = 0; i < other_char_len; i++) {
			System.out.printf("(%d %d)\t", other_char_vec_pos[i], other_char_vec_ch[i]);
        }
		System.out.println();
		
		for (int i = 0; i < line_break_len; i++) {
			System.out.printf("%d\t", line_break_vec[i]);
        }
    }
    ...
    public static void preProcessRef() { //kMerHashing Construct
        for (int i = 0; i < MAX_ARR_NUM; i++) { //1<<30
            point[i] = -1;
        }
        long value = 0;
        for (int k = SUB_STR_LEN - 1; k >= 0; k--) { //19 to 0
            value <<= 2;
            value += ref_seq_code[k];
        }
        int id = (int)(value&(long)(MAX_ARR_NUM - 1)); //  (1<<30) -1
        loc_next[0] = point[id];
        point[id] = 0;
    ...
    public static void searchMatch(String resFile) throws IOException  { //CodeFirstMatch
        preProcessRef();

        StringBuilder strBuf = new StringBuilder(1<<10); //1<<30
        ...
        for (int i =0; i < step_len; i++) {
            ...
            int id = point[(int)(tar_value&(long)(MAX_ARR_NUM - 1))]; //refBucket
            if (id > -1) {
...
*/
    /*public static void main(String[] args) throws IOException, InterruptedException {
        //run example: java -Xmx20g Algorithm ../HG18 ../KOREF20090224 KOREF20090224_ref_HG18
        String fold_list[] = new String[]{"sacCer2", "sacCer3"};

        for (int ri = 0; ri < 6; ri++) {
            for (int ti = 0; ti < 6; ti++) {
                if (ri != ti) {
                    String reference_fold = fold_list[ri]; //sacCer2
                    String target_fold = fold_list[ti]; //sacCer3
                    ...
                    String fileName[] = new String[]{"sacCer2_chrI.fa", "sacCer2_chrII.fa", "sacCer2_chrIII.fa", "sacCer2_chrIV.fa", 
                    "sacCer2_chrV.fa", "sacCer2_chrVI.fa", "sacCer2_chrVII.fa", "sacCer2_chrVIII.fa", "sacCer2_chrIX.fa", "sacCer2_chrX.fa", 
                    "sacCer2_chrXI.fa", "sacCer2_chrXII.fa", "sacCer2_chrXIII.fa", "sacCer2_chrXIV.fa", "sacCer2_chrXV.fa", "sacCer2_chrXVI.fa"};
                    
                    for (int i = 0; i < 24; i++) {
                        compressFile(reference_fold+'/'+fileName[i], target_fold+'/'+fileName[i], final_fold+'/'+fileName[i]);
                    }
					//YH/chr1.fa, HG18/chr1.fa, HG18_ref_YH/chr1.fa
					
                    //./7za a YH_ref_HG18.7z YH_ref_HG18 -m0=PPMD
	...
	String final_fold="Result";
	Runtime.getRuntime().exec("mkdir " + final_fold);

        String fileName[] = new String[]{"Ref.fa","Seq1.fa","Seq2.fa"};
		for(int i=1;i<fileName.length;i++)
			compressFile(fileName[0], fileName[i], final_fold+'/'+fileName[i]);
		

	//PPMD->7zip	
	//String command = "./home/ubuntu/SBR/HiRGC/7za a " + final_fold + ".7z " + final_fold + " -m0=PPMD";
