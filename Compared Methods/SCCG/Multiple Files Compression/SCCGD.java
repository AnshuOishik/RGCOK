...
public class SCCGD {
	...
	public static String readrefSeq(String sequenceFileName) throws IOException {
		...
		while ((line = bufferedreader.readLine()) != null) {
			...
			for (int i = 0; i < line_length; i++) {
				...
				if (!Character.isUpperCase(temp_ch)) {//A,a
	...
	public static void main(String[] args) throws IOException {
		...
		//String ref="hg17_chr5.fa";
		String ref="R.fa";

		String chrName[] = new String[] 
{"T1.fa.7z","T2.fa.7z","T3.fa.7z"};
		/*"K131_chr5.consensus_fasta.7z",
		"HuRef_chr5.fasta.7z",
		"hg19_chr5.fa.7z",
		"hg18_chr5.fa.7z",
		"K224_chromosome_5.fa.7z",
		"YH_chr5.fa.7z",
		"hg38_chr5.fa.7z"};*/

		String reference_file = ref;
		Date startDate = new Date();
		long startCpuTimeNano = getCpuTime();
		System.out.println("Start time: " + startDate);
		String[] chrs={};
	for (int i = 0; i < 3; i++) {
		String in_file_7z = chrName[i];
		chrs = in_file_7z.split("/");
		//String final_file = args[2] + "/" + chrs[(chrs.length-1)];
		String final_file = args[0] + "/" + chrs[(chrs.length-1)].replaceAll(".7z", "");
		
		//String in_file_name = final_file.replaceAll(".fa", ".txt");
				
		

		System.out.println(chrName[i] + " is decompressing...");

		File file = new File(final_file);
		//file.delete();

		use7zip(args[0]+"/"+in_file_7z, args[0]+"/");
		
		//System.out.println(args[0]+"/"+in_file_7z);
...
//		}
		...
		System.out.println("Decompressed file is " + args[0] + "/" + chrs[(chrs.length-1)]);
