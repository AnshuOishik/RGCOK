...
class RGCOKCompress {
    static int max_seq_num = 2000, max_cha_num = 1 << 28, p = 25, k_mer_len = 10;   //max_cha_num = 1 << 28, Optimal k_mer_len value is selected using MtKmer algorithm                     
	static int kMer_bit_num = 2 * k_mer_len, hash_table_len = 1 << kMer_bit_num;     
	static final int max_arr_num_bit = 30, max_arr_num = 1<<max_arr_num_bit; //max_arr_num_bit=30
	...
	static void saveSplInfo(BufferedWriter bw, GenSequence seq) {
        ...
        int flag = 0;
        try {
			// Modified RLE For lower-case character matching
            if (seq_low_vec_len > 0 && ref_low_vec_len > 0) { 
                if ((2 * seq.getDiffLowVecLen()) < seq_low_vec_len) { //2*1 < 3
                    flag = 1;
                    bw.write(flag + " ");
			...
			// Modified RLE For N/n character matching (Encode if c.ratio improved)
			flag = 0;
			...
			//Without Modified RLE for n or N character
			/*savePosInfo(bw, seq_n_vec_len, seq.getNVecBegin(), seq.getNVecLength()); 
			seq.setNVecLen(0);
            seq.setNVecBegin(null);
            seq.setNVecLength(null);*/
	...
	static int getTempHashValue(MatEntry me) { 
		...
		//System.out.println(str+" "+me.getPos()+" "+me.getLen());
        ...
        for (i = 1; i < step_len; i++) { 
            ...
            if (id > -1) {
                ...
                if (max_len > -1) {
					...
					//System.out.println(mis_mat_str);
                    me.setPos(max_k - pre_pos); 
                    me.setLen(max_len - min_rep_len); 
	//System.out.println(mis_mat_str+" "+max_k+" "+pre_pos+" "+max_len+" "+min_rep_len);
                    mat_res.add(me);   
                    me = new MatEntry();
                    i += max_len;
    ...
    static void tarSeqExt(int seq_num, GenSequence seq) { 
        ...
        try {
            ...
            if (n_flag)
                seq_n_vec_length[seq_n_vec_len++] = n_lett_len;
			/*for(int i=0;i<seq_code_len;i++)
				System.out.print(seq_code[i]);
			System.out.println();*/
			br.close();
        } catch (Exception e) { 
            System.out.println(e);
    ...
    static void eRLELowCase(BufferedWriter bw, int[] vec, int len) { 
		...
		for (int i = 0; i < len; i++)
			System.out.print(vec[i]+" "); //0 2 3
		System.out.println(); */
        List<Integer> code = new ArrayList<>(vec_size); //1<<20
        int i, c, code_len;
		if (len > 0) { 
            code.add(vec[0]); //0
            //c = 1;
			c = 0;
            for (i = 1; i < len; i++) { 
                if (vec[i] - vec[i - 1] == 1) //2-0=2, 3-2=1
                    c++; //2
                else {
                    code.add(c); //1
                    code.add(vec[i]); //2
    ...
    static void savePosInfo(BufferedWriter bw, int vec_len1, int[] vecBegin, int[] vec_len2) { //bw, 1, 12, 10; bw, 2, 19, 3
        int i;
		/*System.out.println();
		System.out.println(vec_len1);
		for (i = 0; i < vec_len1; i++)
			System.out.println(vecBegin[i] + " " + vec_len2[i]);*/
		try {
            bw.write(vec_len1 + " "); 
            for (i = 0; i < vec_len1; i++)
    ...
    static void saveMatRes(BufferedWriter bw, MatEntry me) {
        try {
			...
			//System.out.println(me.getMisMatStr() + " " + me.getPos() + " " + me.getLen());
        } catch (IOException e) {
            System.out.println(e);
        }
    }
	...
	static void refSeqExt(String ref_path) {
        ...
        try {
			...
			if (n_flag) 
                ref_n_vec_length[ref_n_vec_len++] = n_lett_len;
			br.close();
			/*for(i=1;i<ref_low_vec_len;i++)
				System.out.println(ref_low_vec_begin[i]+ " "+ref_low_vec_length[i]);
			for(i=1;i<ref_n_vec_len;i++)
				System.out.println(ref_n_vec_begin[i]+" "+ref_n_vec_length[i]);
			System.out.println();*/
        } catch (Exception e) {
             System.out.println(e);
        } 
	...
	static void seqCompress(String f_path, int pool_size) { 
        ...
        try {
            ...
            for (i = 1; i < seq_number; i++) { 
                ...
                br = new BufferedReader(new FileReader(f));
                /*while ((str = br.readLine()) != null) //Original
                    bw1.write(str + "\n"); 
                bw1.write("\n"); */
