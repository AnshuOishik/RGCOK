...
class RGCOKCompress {
	...
	static int kMer_bit_num = 2 * k_mer_len, hash_table_len;     
	...
	static String stringToASCIICode(String mis_mat_str){
		...
		for (i =0; i < step_len; i++) {
			...
			//Character 10 and 13 creates problem, 32 (space) as RME are spilit w.r.t space, 65 (A), 67 (C), G (71), T (84), 48 to 57 for count in RLE
			if(miss_match_value == 10 || miss_match_value == 13|| miss_match_value == 32 || miss_match_value == 65 || miss_match_value == 67 || miss_match_value == 71 || miss_match_value ==84|| (miss_match_value >= 48 && miss_match_value <= 57) ) 
				for (k = 0; k <= 3; k++){
	...
	static void kMerHash() { 
		...
		if(k_mer_len>15)
			hash_table_len = 1 << max_arr_num_bit_match;
		else
