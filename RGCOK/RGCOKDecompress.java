...
class RGCOKDecompress {
    static int max_seq_num = 2000, max_cha_num = 1 << 28, p = 25, k_mer_len = 10; //max_cha_num = 1 << 28 (Optimal)
    static int kMer_bit_num = 2 * k_mer_len, hash_table_len = 1 << kMer_bit_num; 
	static int max_arr_num_bit = 30, max_arr_num = 1<<max_arr_num_bit; //max_arr_num_bit=30
	...
	static int[] ref_loc, ref_bucket, ref_low_vec_begin, ref_low_vec_length, ref_n_vec_begin, ref_n_vec_length;
	...
	static void readCompTarSeq(GenSequence seq) {
        ...
        for (i = 0; i < mat_res.size(); i++) {
            ...
            for (j = 0; j < mis_str.length(); j++){
				...
				//System.out.print(mis_str.charAt(j));
			}
	...
	static void getMatRes(int id, int pos, int l, List<MatEntry> mat_res) {
        for (int i = 0; i < l; i++){
			//System.out.println(match_res_vec.get(id).get(pos));
	...
	static void readSplInfo(GenSequence seq) {
        try {
            ...
            if (flag1 == 0) {
                ...
                for (i = 2; i < 2 + seq_low_vec_len * 2; i++) { //2 to 7
                    if (i % 2 == 0) 
				...
				flag2 = Integer.valueOf(str[2 + seq_low_vec_len * 2]); //8
				if(flag2 == 0){
					int seq_n_vec_len = Integer.valueOf(str[3 + seq_low_vec_len * 2]); //9
					k = 0;
					...
					int j, l = Integer.valueOf(str[3 + seq_low_vec_len * 2]); //9 -> 4
					List<Integer> v = new ArrayList<>(vec_size);
					for (i = 4 + seq_low_vec_len * 2; i < 4 + seq_low_vec_len * 2 + l; i++) //10 to 13
						v.add(Integer.valueOf(str[i])); //0 0 2 1
						
					int seq_n_vec_len = 0;
					for (i = 1; i < v.size(); i += 2){ //1 to 4
						//seq_n_vec_len += v.get(i); //0+1 = 1+2 = 3
						seq_n_vec_len += v.get(i)+1; //0+1 = 1+1+1 = 3
					}
					
					int[] seq_n_vec_matched = new int[seq_n_vec_len]; //3
					k = 0;
					for (i = 0; i < v.size(); i += 2) //0 2
						for (j = 0; j < v.get(i + 1)+1; j++) //<1, <2
							seq_n_vec_matched[k++] = v.get(i) + j; //0 2 3
					
					int seq_diff_n_vec_len = Integer.valueOf(str[4 + seq_low_vec_len * 2 + l]); //10+4 = 14 -> 1
					k = 0;
					int[] seq_diff_n_vec_begin = new int[seq_diff_n_vec_len]; //1
					int[] seq_diff_n_vec_length = new int[seq_diff_n_vec_len]; //1
					for (i = 5 + seq_low_vec_len * 2 + l; i < 5 + seq_low_vec_len * 2 + l + seq_diff_n_vec_len * 2; i++) { 
						if (i % 2 == 1) 
							seq_diff_n_vec_begin[k] = Integer.valueOf(str[i]); //9
						else
							seq_diff_n_vec_length[k++] = Integer.valueOf(str[i]); //6
					}
						
					k = 0;
					int[] seq_n_vec_begin = new int[vec_size];
					int[] seq_n_vec_length = new int[vec_size];
					for (i = 0, j = 0; i < seq_n_vec_len; i++) { //<3
						k = seq_n_vec_matched[i];
						if (k == 0) {
							seq_n_vec_begin[i] = seq_diff_n_vec_begin[j]; //9
							seq_n_vec_length[i] = seq_diff_n_vec_length[j++]; //6
						} else {
							seq_n_vec_begin[i] = ref_n_vec_begin[k]; //4, 7
							seq_n_vec_length[i] = ref_n_vec_length[k]; //3, 7 
						}
					}
					seq.setNVecLen(seq_n_vec_len);
					seq.setNVecBegin(seq_n_vec_begin);
					...
					int seq_spe_cha_len = Integer.valueOf(str[5 + l + seq_diff_n_vec_len * 2 + seq_low_vec_len * 2]); //17 -> 3
					k = 0;
					if (seq_spe_cha_len > 0) {
						int[] seq_spe_cha_pos = new int[seq_spe_cha_len];
						int[] seq_spe_cha_ch = new int[seq_spe_cha_len];
            ...
            } else { //flag1=1, 1 4 0 1 2 2
                int i, j, l = Integer.valueOf(str[1]);
                List<Integer> v = new ArrayList<>(vec_size);
                for (i = 2; i < 2 + l; i++) //2 to 5
                    v.add(Integer.valueOf(str[i]));
				
                int seq_low_vec_len = 0;
                for (i = 1; i < v.size(); i += 2) //1 to 3
                    seq_low_vec_len += v.get(i)+1; //0+1=1+1+1=3
				//System.out.println(seq_low_vec_len);
				...
				flag2 = Integer.valueOf(str[3 + l + seq_diff_low_vec_len * 2]); //9
				if(flag2 == 0){
					...
					int l1 = Integer.valueOf(str[4 + l + seq_diff_low_vec_len * 2]); //6 -> 2
					v = new ArrayList<>(vec_size);
					for (i = 5 + l + seq_diff_low_vec_len * 2; i < 5 + l + seq_diff_low_vec_len * 2 + l1; i++) //10 to 13
						v.add(Integer.valueOf(str[i])); //1 3
						
					int seq_n_vec_len = 0;
					for (i = 1; i < v.size(); i += 2) //1 to <2
						seq_n_vec_len += v.get(i)+1; //0+3 = 3
					
					int[] seq_n_vec_matched = new int[seq_n_vec_len]; //3
					k = 0;
					for (i = 0; i < v.size(); i += 2) //0 to <2
						for (j = 0; j < v.get(i + 1)+1; j++) //0 to <3
							seq_n_vec_matched[k++] = v.get(i) + j; //1 2 3
					
					int seq_diff_n_vec_len = Integer.valueOf(str[5 + l + seq_diff_low_vec_len * 2 + l1]); //5+2+0+2=9 -> 0
					k = 0;
					int[] seq_diff_n_vec_begin = new int[seq_diff_n_vec_len]; //0
					int[] seq_diff_n_vec_length = new int[seq_diff_n_vec_len]; //0
					...
					int seq_spe_cha_len = Integer.valueOf(str[6 + l + seq_diff_low_vec_len * 2 + l1 + seq_diff_n_vec_len * 2]); //10 -> 3
					k = 0;
					if (seq_spe_cha_len > 0) {
						int[] seq_spe_cha_pos = new int[seq_spe_cha_len];
						int[] seq_spe_cha_ch = new int[seq_spe_cha_len];
	...
	//Without Modified RLE on N/n char
	/*static void readSplInfo(GenSequence seq) {
        try {
            ...
            if (flag == 0) {
				...
				int seq_n_vec_len = Integer.valueOf(str[2 + seq_low_vec_len * 2]); //8
                k = 0;
                int[] seq_n_vec_begin = new int[seq_n_vec_len];
                int[] seq_n_vec_length = new int[seq_n_vec_len];
                ...
                int seq_low_vec_len = 0;
                for (i = 1; i < v.size(); i += 2) //1 to 3
                    seq_low_vec_len += v.get(i); //0+1=1+2=3
				System.out.println(seq_low_vec_len);
                int[] seq_low_vec_matched = new int[seq_low_vec_len];
                int k = 0;
                for (i = 0; i < v.size(); i += 2)
	...
	public static void bscDecompression() {
        try {
            ...
            p2.waitFor();
            String delCommand = "rm -f " + "TarC.tar";
